"""
CogOS — Production-Grade Baseline (single-file reference implementation)
...
class MemoryStore:
    ...
    def _from_blob(self, b: Optional[bytes], dim: Optional[int]) -> "np.ndarray":
        if b is None or dim is None:
            return np.zeros((self.embedder.dim,), dtype=np.float32)
    ...
    def _vector_candidates(self, table: str, query_vec: "np.ndarray", k: int) -> List[Tuple[str, float]]:
        with self._lock:
            rows = self._conn.execute(f"SELECT id, emb, emb_dim FROM {table}").fetchall()
        scored: List[Tuple[str, float]] = []
        for r in rows:
            v = self._from_blob(r["emb"], r["emb_dim"])
            s = cosine(query_vec, v)
            scored.append((str(r["id"]), float(s)))
...
class LLMReasoner(Reasoner):
    ...
    def propose(self, user_text: str, *, plan: Plan, evidence_map: Dict[str, str], memory_hits: Dict[str, Any], tool_outcomes: List["ToolOutcome"]) -> ProposedAnswer:
        ...
        for eid, txt in list(evidence_map.items())[:12]:
            blocks.append(f"[{eid}] {short(txt, 1600)}")
...
class CogOS:
    def __init__(self, cfg: AgentConfig):
        ...
        if cfg.background:
            ...
            self.bg = BackgroundRunner(self.bus, ctx, [ReflectionDaemon(), ConnectionMiner(), ConsistencyAuditor(), TaskSolverDaemon()])
            self.bg.start()
